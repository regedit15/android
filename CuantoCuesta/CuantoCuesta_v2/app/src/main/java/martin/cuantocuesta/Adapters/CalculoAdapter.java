...
import martin.cuantocuesta.Services.UtilService;
import martin.cuantocuesta.Services.UtilServiceLocal;
import martin.cuantocuesta.inputsFilters.UtilInputFilter;
		...
		public void bind(final Calculo calculo) {
			...
			btTipo.setOnClickListener(view -> {
				...
				builder.setSingleChoiceItems(unidades, Arrays.asList(unidades).indexOf(calculo.getUnidad()), (dialog1, index) -> {
					...
					String ultimoTipoDeUnidadNueva = unidades[indexUnidad.get()];

					// se borra la cantidad si estaba en kilo o litro y se puso otro que no sea kilo o litros. Ya que si escribió 1.5 kilos y lo pasa a gramos no se puede
					// ultimoTipoDeUnidad = kilo
					// ultimoTipoDeUnidadNueva = litro
					// valor = 1.5
					if (
							tiCantidad.getText().toString().contains(".")
									&& (ultimoTipoDeUnidad.equals(UtilServiceLocal.TIPO_KILO) || ultimoTipoDeUnidad.equals(UtilServiceLocal.TIPO_LITRO))
									&& (!ultimoTipoDeUnidadNueva.equals(UtilServiceLocal.TIPO_KILO) && !ultimoTipoDeUnidadNueva.equals(UtilServiceLocal.TIPO_LITRO))
					) {
						tiCantidad.setText("");
					}

					ultimoTipoDeUnidad = ultimoTipoDeUnidadNueva;
